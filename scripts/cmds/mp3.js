...
module.exports.config = {
  ...
  onStart: async ({ event, api, config }) => {
    ...
    const waitingMsg = await message.reply(chatId, "⏳ Please wait while your request is being processed...");
    ...
    if (msg.reply_to_message && msg.reply_to_message.video) {
      ...
      try {
        ...
        await api.deleteMessage(chatId, waitingMsg.message_id);
        message.reply(chatId, "❌ An error occurred while processing the video.");
      }
    } else if (url) {
      try {
        console.log(url);
        ...
        if (data.status && data.data && data.data.audio) {
          ...
          await api.deleteMessage(chatId, waitingMsg.message_id);
          message.reply(chatId, "❌ Failed to retrieve audio. Please check the URL or try again.");
        }
      } catch (error) {
        console.error('Error processing the URL:', error);
        await api.deleteMessage(chatId, waitingMsg.message_id);
        message.reply(chatId, "❌ An error occurred while processing the URL.");
      }
    } else {
      await api.deleteMessage(chatId, waitingMsg.message_id);
      message.reply(
